...
```
Add the Westline style selector feature to this project so the client can
...
2. Read node_modules/@westline/style-selector/GUIDE.md for the full
...
5. Create 4 presets in src/style-selector/presets/[surface].ts
...
6. Create src/style-selector/config.ts with:
...
7. Mount in the layout:
   ...
   - <StyleSelector config={...} /> after the theme wrapper
...
8. Add NEXT_PUBLIC_ENABLE_STYLE_SELECTOR=true to .env.local
...
4. Create these presets in src/style-selector/presets/[surface].ts:
   - Default: [CURRENT THEME NAME] — empty variables: {}
   - [NAME]: [DESCRIPTION]
   - [NAME]: [DESCRIPTION]
   - [NAME]: [DESCRIPTION]

5. Create src/style-selector/config.ts (use instanceId, not storageKey,
   ...
   Mount PrepaintScript + StyleSelector in the layout,
...
- **Source copying** — The picker engine lives in the installed `@westline/style-selector` package. Only `src/style-selector/*` stays in the consuming repo.
...
Don't use this prompt. The playbooks already include style selector:
