...
import { getStoredPresetId, storePresetId, clearStoredPreset, getPresetIdFromUrl, buildPreviewUrl, clearUrlParam, } from '../core/persistence.js';
export function useStylePreview(config, options = {}) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            for (const preset of config.presets) {
                if (!validatePreset(preset, config.allowedTokens)) {
                    console.warn(`[style-preview] Preset "${preset.id}" fails validation against allowedTokens.`, preset);
                }
            }
        }
    ...
    const previewUrl = useMemo(() => buildPreviewUrl(activePresetId, config.defaultStyleId, config.queryParam), [activePresetId, config.defaultStyleId, config.queryParam]);
    return {
        activePresetId,
        targetFound,
        setPreset,
