...
export function PreviewTrigger({ onOpen, onClose, drawerOpen }) {
    const handleClick = drawerOpen ? onClose : onOpen;
    return (_jsxs("button", { onClick: handleClick, "aria-label": drawerOpen ? 'Close style preview' : 'Open style preview', style: {
            all: 'initial',
            position: 'fixed',
            right: '0',
            top: '50%',
            ...
            const stripe = e.currentTarget.querySelector('[data-stripe]');
            if (stripe)
            ...
            const stripe = e.currentTarget.querySelector('[data-stripe]');
            if (stripe)
                stripe.style.opacity = drawerOpen ? '0.4' : '1';
        }, onFocus: (e) => {
            e.currentTarget.style.outline = `2px solid ${WM.green}`;
            e.currentTarget.style.outlineOffset = '2px';
        }, onBlur: (e) => {
            e.currentTarget.style.outline = 'none';
        }, children: [_jsx("span", { "aria-hidden": "true", "data-stripe": "", style: {
                    position: 'absolute',
                    left: '0',
                    top: '12px',
                    bottom: '12px',
                ...
                } }), _jsx("svg", { width: "8", height: "14", viewBox: "0 0 8 14", fill: "none", "aria-hidden": "true", style: {
                    display: 'block',
                    marginLeft: '2px',
                    flexShrink: 0,
                    transform: drawerOpen ? 'rotate(180deg)' : 'none',
                    transition: 'transform 0.2s ease',
                }, children: _jsx("path", { d: "M6.5 1.5L2 7l4.5 5.5", stroke: WM.gray, strokeWidth: "1.5", strokeLinecap: "round", strokeLinejoin: "round" }) })] }));
}
