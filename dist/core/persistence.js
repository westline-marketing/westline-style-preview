...
export function getStoredPresetId(options = {}) {
    ...
    try {
        ...
        if (namespacedPresetId)
            return namespacedPresetId;
        if (!legacyStorageKey)
            return null;
...
export function storePresetId(id, options = {}) {
    if (typeof window === 'undefined')
        return;
    const storageKey = resolveStorageNamespace(options);
    try {
...
export function clearStoredPreset(options = {}) {
    if (typeof window === 'undefined')
        return;
    const storageKey = resolveStorageNamespace(options);
    const legacyStorageKey = resolveLegacyStorageKey(options);
...
export function getPresetIdFromUrl(param = DEFAULT_QUERY_PARAM) {
    if (typeof window === 'undefined')
        return null;
    try {
...
export function buildPreviewUrl(presetId, defaultId, param = DEFAULT_QUERY_PARAM) {
    if (typeof window === 'undefined')
        return '';
    const url = new URL(window.location.href);
...
export function clearUrlParam(param = DEFAULT_QUERY_PARAM) {
    if (typeof window === 'undefined')
        return;
    try {
        const url = new URL(window.location.href);
