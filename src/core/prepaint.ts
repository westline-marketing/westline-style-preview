...
export function generatePrepaintScript(config: PreviewConfig): string {
  ...
  for (const preset of config.presets) {
    const vars = Object.fromEntries(Object.entries(preset.variables).filter(
      ([k]) => !allowed || allowed.includes(k)
    ))
    if (Object.keys(vars).length === 0) continue
    presetMap[preset.id] = vars
  }

  if (Object.keys(presetMap).length === 0) return ''
